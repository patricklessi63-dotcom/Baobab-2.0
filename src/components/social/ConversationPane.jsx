...
import { ArrowLeft, MoreVertical, Flag, Ban, Send, Check, CheckCheck, Circle, ShieldAlert, ShieldCheck, RotateCcw } from "lucide-react";
...
export default function ConversationPane({
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        {messages.map((m, i) => {
          ...
          return (
            <React.Fragment key={m.id}>
              ...
              <div
                className="motion-safe:transition-opacity max-w-[75%] text-sm px-3.5 py-2.5 rounded-2xl flex items-end gap-1.5"
                style={{
                  ...(isMine
                    ? { alignSelf: "flex-end", background: primary, color: "#F7F8FA", borderBottomRightRadius: 4 }
                    : { alignSelf: "flex-start", background: bg, color: "#20243A", borderBottomLeftRadius: 4 }),
      ...
      <div className="p-4 flex gap-2 shrink-0 sticky bottom-0 bg-white" style={{ borderTop: "1px solid rgba(21,27,61,.08)", paddingBottom: "max(1rem, env(safe-area-inset-bottom))" }}>
        <textarea
          value={messageDraft}
