...
export default function Step4CanadaJourney({ draft, update }) {
  return (
    <div className="flex flex-col gap-3">
      ...
      <label className="text-xs font-semibold" style={{ color: "rgba(43,36,32,0.55)" }}>Depuis combien de temps vis-tu au Canada ?</label>
      ...
      <label className="text-xs font-semibold mt-1" style={{ color: "rgba(43,36,32,0.55)" }}>Situation actuelle</label>
      <ChipSelect options={IMMIGRATION_STATUS_OPTIONS} value={draft.immigrationStatus} onChange={(v) => update({ immigrationStatus: v })} />

      <label className="text-xs font-semibold mt-1" style={{ color: "rgba(43,36,32,0.55)" }}>Profession</label>
      ...
      <label className="text-xs font-semibold mt-1" style={{ color: "rgba(43,36,32,0.55)" }}>Études</label>
      <ChipSelect options={EDUCATION_LEVELS} value={draft.educationLevel} onChange={(v) => update({ educationLevel: v })} />

      <label className="text-xs font-semibold mt-1" style={{ color: "rgba(43,36,32,0.55)" }}>Ville d'arrivée au Canada (facultatif)</label>
