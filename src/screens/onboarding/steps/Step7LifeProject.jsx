...
export default function Step7LifeProject({ draft, update }) {
  return (
    <div className="flex flex-col gap-3">
      ...
      <p className="text-xs font-semibold mt-1" style={{ color: "rgba(43,36,32,0.55)" }}>Souhaites-tu avoir des enfants ?</p>
      <ChipSelect options={WANTS_CHILDREN_OPTIONS} value={draft.wantsChildren} onChange={(v) => update({ wantsChildren: v })} />

      <p className="text-xs font-semibold mt-2" style={{ color: "rgba(43,36,32,0.55)" }}>Importance de la famille</p>
      <ChipSelect options={FAMILY_IMPORTANCE_OPTIONS} value={draft.familyImportance} onChange={(v) => update({ familyImportance: v })} />

      <p className="text-xs font-semibold mt-2" style={{ color: "rgba(43,36,32,0.55)" }}>Projet professionnel</p>
      <ChipSelect options={CAREER_GOAL_OPTIONS} value={draft.careerGoal} onChange={(v) => update({ careerGoal: v })} />

      <p className="text-xs font-semibold mt-2" style={{ color: "rgba(43,36,32,0.55)" }}>Projet géographique</p>
