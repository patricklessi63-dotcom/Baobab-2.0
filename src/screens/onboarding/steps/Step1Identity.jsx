...
export default function Step1Identity({ draft, update }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <label className="text-xs font-semibold" style={{ color: "rgba(43,36,32,0.55)" }}>Prénom</label>
      <input
        value={draft.name}
      ...
      <label className="text-xs font-semibold mt-1" style={{ color: "rgba(43,36,32,0.55)" }}>Date de naissance</label>
      <input
        type="date"
      ...
      {draft.birthDate && age !== null && age >= 18 && age <= 100 && (
        <p className="text-xs" style={{ color: "rgba(43,36,32,0.5)" }}>Âge affiché sur ton profil : {age} ans. Ta date de naissance complète n'est jamais visible publiquement.</p>
      )}
    </div>
  );
}
