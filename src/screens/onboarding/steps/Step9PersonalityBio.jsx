...
export default function Step9PersonalityBio({ draft, update }) {
  return (
    <div className="flex flex-col gap-3">
      ...
      <p className="text-xs font-semibold mt-1" style={{ color: "rgba(43,36,32,0.55)" }}>Je préfère :</p>
      <ChipSelect options={PERSONALITY_EVENING_OPTIONS} value={draft.personalityEvening} onChange={(v) => update({ personalityEvening: v })} />

      <p className="text-xs font-semibold mt-2" style={{ color: "rgba(43,36,32,0.55)" }}>Quand je voyage :</p>
      <ChipSelect options={PERSONALITY_TRAVEL_OPTIONS} value={draft.personalityTravel} onChange={(v) => update({ personalityTravel: v })} />

      <p className="text-xs font-semibold mt-2" style={{ color: "rgba(43,36,32,0.55)" }}>Pour moi, une bonne relation repose surtout sur (max 2) :</p>
      ...
      <p className="text-xs font-semibold mt-3" style={{ color: "rgba(43,36,32,0.55)" }}>Parle-nous de toi</p>
      <textarea
        value={draft.bio}
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs" style={{ color: "rgba(43,36,32,0.45)" }}>
          {BIO_PROMPTS.join(" · ")}
        </p>
        <span className="text-xs shrink-0 ml-2" style={{ color: "rgba(43,36,32,0.4)" }}>{(draft.bio || "").length}/{BIO_MAX}</span>
      </div>
    </div>
  );
}
